...
export default function PDFDialogContent({ url, downloadName, title, description }: PDFDialogContentProps) {
  ...
  return (
    <DialogContent className="top-0 left-0 flex h-svh w-full max-w-full translate-x-0 translate-y-0 flex-col rounded-none p-4 sm:top-[50%] sm:left-[50%] sm:h-auto sm:w-auto sm:max-w-5xl sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-lg sm:p-6">
      <DialogHeader>
      ...
      <div className="overflow-y-auto border sm:max-h-[70vh]">
        <Document file={url} onLoadSuccess={onDocumentLoadSuccess}>
          {Array.from({ length: numPages }, (_, index) => (
            <Page key={`page-${index + 1}`} pageNumber={index + 1} width={960} />
          ))}
        </Document>
      </div>

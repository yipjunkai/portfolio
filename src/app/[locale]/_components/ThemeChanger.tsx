...
export default function ThemeChanger({ className, style }: HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <div
      className={cn(
        "relative flex w-min items-center gap-2 overflow-visible rounded-full bg-linear-to-r from-grad-1 to-grad-2 p-1 text-neutral-200 before:absolute before:-inset-1 before:z-0 before:animate-[gradient-rotate_2s_ease-in-out_infinite] before:rounded-full before:bg-[linear-gradient(to_right,var(--color-grad-1),var(--color-grad-1),var(--color-grad-3),var(--color-grad-3),var(--color-grad-1),var(--color-grad-1),var(--color-grad-3),var(--color-grad-3),var(--color-grad-1))] before:bg-size-[200%_100%] before:opacity-80 before:blur-md before:content-[''] motion-reduce:before:animate-none lg:gap-1 lg:before:animate-none lg:before:opacity-0",
        className
      )}
      style={style}
    >

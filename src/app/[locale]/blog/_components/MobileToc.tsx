...
export default function MobileToc({ headings, label }: { headings: TocEntry[]; label: string }) {
  ...
  return (
    <details className="group rounded-lg border border-neutral-300 lg:hidden dark:border-neutral-700">
      ...
      <ul className="flex flex-col gap-1 border-t border-neutral-200 px-4 py-3 dark:border-neutral-800">
        {headings.map(heading => (
          <li key={heading.slug}>
            <a
              href={`#${heading.slug}`}
              className="block py-1 text-sm text-neutral-600 transition-colors hover:text-foreground dark:text-neutral-400"
            >
              {heading.text}
            </a>
          </li>
